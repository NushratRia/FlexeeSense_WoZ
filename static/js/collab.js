  ...
  function _applyLinkAdd(data) {
    ...
    if (typeof createLink === "function") {
      ...
      if (newLinkId && newLinkId !== data.linkId && LINKS[newLinkId]) {
        ...
        if (card?.anchors) {
          card.anchors = card.anchors.map((id) =>
            id === newLinkId ? data.linkId : id
          );
        }
      }
    }
  ...
  window.collabToggleVideo = async function () {
    ...
    if (_videoEnabled) {
      ...
      if (stream) {
        ...
        Object.keys(_peerBubbles).forEach(async (sid) => {
          // if (sid !== 'local') _createPeerConn(sid, true);
          if (sid === "local") return;

          let pc = _peerConns[sid];
          if (!pc) {
            pc = await _createPeerConn(sid, true);
            return;
          }
          ...
          //renotiate to notify peer of new tracks
